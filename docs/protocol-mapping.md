...
| Segment names are entered by the user, not invented by the agent | `workflow/segments.json` + the editor on the live page (`POST /api/segments`); `orchestrator.start` adopts the name from that file |
...
| Image generation itself requires prior user confirmation | `requires_user_confirmation` gate on the `short-drama-image-generator` Skill (see `16_生图渠道规则_v3.0.md` §5) |
